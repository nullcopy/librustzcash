...
fn parse_account_source(
    ...
    match (account_kind, derivation) {
        ...
        #[cfg(feature = "zip-48")]
        (2, None) => Ok(AccountSource::Zip48 { key_source }),
...
impl zcash_client_backend::data_api::Account for Account {
    ...
    #[cfg(feature = "zip-48")]
    fn zip48_fvk(&self) -> Option<&::transparent::zip48::FullViewingKey> {
        match &self.viewing_key {
            ViewingKey::Zip48Full(fvk) => Some(fvk),
            _ => None,
        }
    }
}
...
impl ViewingKey {
    ...
    fn uivk(&self) -> UnifiedIncomingViewingKey {
        match self {
            ...
            #[cfg(feature = "zip-48")]
            ViewingKey::Zip48Full(_) => unreachable!(
                "ZIP 48 multisig accounts do not have a unified incoming viewing key; \
                 callers must check for Zip48Full before calling uivk()"
            ),
        }
    }
...
pub(crate) fn add_account<P: consensus::Parameters>(
    ...
    let (derivation, spending_key_available, key_source) = match kind {
        ...
        #[cfg(feature = "zip-48")]
        AccountSource::Zip48 { key_source } => (None, false, key_source),
    };
    ...
    if !is_zip48_multisig {
        ...
        for key_scope in [TransparentKeyScope::EXTERNAL, TransparentKeyScope::INTERNAL] {
            transparent::generate_gap_addresses(
                ...
                account_id,
                key_scope,
                // UnifiedAddressRequest is ignored for ZIP 48 accounts; the ZIP 48
                // code path in generate_address_range returns early before it is used.
                UnifiedAddressRequest::ALLOW_ALL,
                false,
            )?;
        }
...
///
/// Uses the shared gap-address infrastructure to find an unreserved address within the gap,
/// then marks it as exposed.
#[cfg(feature = "zip-48")]
pub(crate) fn get_next_available_zip48_multisig_address<P: consensus::Parameters>(
    ...
    // Return Ok(None) if not a ZIP 48 account
    if account.zip48_fvk().is_none() {
        return Ok(None);
    }

    let t_scope = TransparentKeyScope::from(scope);
    let gap_limit = gap_limits.limit_for(t_scope).ok_or(
        AddressGenerationError::UnsupportedTransparentKeyScope(t_scope),
    )?;

    // Pre-generate gap addresses
    transparent::generate_gap_addresses(
        conn,
        params,
        gap_limits,
    ...
    // Find an unreserved address within the gap (shared infrastructure)
    let (gap_start, addrs) =
        transparent::select_addrs_to_reserve(conn, params, account.id, t_scope, gap_limit, 1)?;

    let (address_id, taddr, meta) =
        addrs
            .into_iter()
            .next()
    ...
    let chain_tip = chain_tip_height(conn)?.ok_or(SqliteClientError::ChainHeightUnknown)?;
    conn.execute(
        "UPDATE addresses SET exposed_at_height = :height WHERE id = :id",
        named_params![":height": u32::from(chain_tip), ":id": address_id.0],
    )?;

    let address_index = meta
        .address_index()
        .expect("gap addresses always have an address index");
    ...
    Ok(Some((
        ...
        TransparentAddressMetadata::zip48_multisig(
            ...
            Exposure::Exposed {
                at_height: chain_tip,
                gap_metadata: GapMetadata::InGap {
...
pub(crate) fn get_zip48_multisig_address_for_index<P: consensus::Parameters>(
    ...
    let fvk = match account.zip48_fvk() {
        Some(fvk) => fvk,
        None => return Ok(None),
    };

    let t_scope = TransparentKeyScope::from(scope);
    let (taddr, _) = fvk.derive_address(scope, address_index);

    // Ensure address is in the database (DO NOTHING if already exists)
    transparent::generate_address_range(
        conn,
        params,
        account.id,
    ...
    let chain_tip = chain_tip_height(conn)?.ok_or(SqliteClientError::ChainHeightUnknown)?;
    conn.execute(
        "UPDATE addresses SET exposed_at_height = MIN(
            IFNULL(exposed_at_height, :height), :height
         ) WHERE account_id = :account_id
           AND key_scope = :key_scope
           AND transparent_child_index = :index",
        named_params![
            ":height": u32::from(chain_tip),
            ":account_id": account.id.0,
            ":key_scope": KeyScope::try_from(t_scope)?.encode(),
