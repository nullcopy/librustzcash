    ...
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    ...
    let account = st
        .wallet_mut()
        .import_account_zip48_multisig("zip48-test", &fvk, &birthday, None)
        .unwrap();
...
pub fn put_received_transparent_utxo_derived<DSF>(dsf: DSF)
where
    DSF: DataStoreFactory,
    ...
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    ...
    put_received_transparent_utxo::<_, DSF>(&mut st, account_id);
}

#[cfg(feature = "zip-48")]
pub fn put_received_transparent_utxo_zip48<DSF>(dsf: DSF)
where
    DSF: DataStoreFactory,
    <<DSF as DataStoreFactory>::DataStore as WalletWrite>::UtxoRef: std::fmt::Debug + PartialEq,
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    ...
    let (account_id, _, _, _) = create_zip48_test_account::<_, DSF>(&mut st);
    put_received_transparent_utxo::<_, DSF>(&mut st, account_id);
}

fn put_received_transparent_utxo<Cache, DSF>(
    ...
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    let taddr = *st
        ...
        .next()
        .unwrap();
    let birthday = st.wallet().get_account_birthday(account_id).unwrap();
...
pub fn transparent_balance_spendability_derived<DSF>(dsf: DSF, cache: impl TestCache)
where
    DSF: DataStoreFactory,
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
    let mut st = TestBuilder::new()
        ...
        .build();
    let account_id = st.test_account().unwrap().id();
    transparent_balance_spendability::<DSF>(&mut st, account_id);
}

#[cfg(feature = "zip-48")]
pub fn transparent_balance_spendability_zip48<DSF>(dsf: DSF, cache: impl TestCache)
where
    DSF: DataStoreFactory,
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
{
...
fn transparent_balance_spendability<DSF>(
    st: &mut TestState<impl TestCache, <DSF as DataStoreFactory>::DataStore, LocalNetwork>,
    account_id: <DSF as DataStoreFactory>::AccountId,
) where
    DSF: DataStoreFactory,
    <DSF as DataStoreFactory>::AccountId: std::fmt::Debug,
